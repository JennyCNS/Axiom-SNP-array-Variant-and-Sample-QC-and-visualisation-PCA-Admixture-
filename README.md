...
names(tbl3)[bestKvalue+1] <- "ind"

# #create a vector to group individuals (e.g. by population)
spp <- rep (NA, length(pca$ind))
spp[grep("YourSampleName1", tbl3$ind)] <- "Pop_A"
spp[grep("YourSampleName2", tbl3$ind)] <- "Pop_B"

k <- as_tibble(data.frame(tbl3,spp))

#you can reorder your samples on the X axis by using the following command line
new_order <- c("Pop_A", "Pop_B", "...")
k_temp$spp <- factor(as.character(k_temp$spp), levels=new_order)
k <- k[order(k3$spp),]
